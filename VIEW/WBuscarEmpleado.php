<?php
	...
	$aColumns = array( 'id_persona', 'nom_persona','ruc_persona','telf_persona','cel_persona','email_persona','estado_persona','id_tipoper', 'ape_persona' );
	...
	if ( $_GET['sSearch'] != "" )
	{
		...
		$sWhere .= ')'." AND id_tipoper IN(1,4)";
	}else{
         $sWhere = " WHERE id_tipoper IN(1,4) ";   
        }
	...
	$sQuery = "
		...
		FROM   $sTable WHERE id_tipoper IN(1,4)";
	...
	{
		if($aRow[ $aColumns[7] ] == "4")
			$aRow[ $aColumns[7] ] = "MECANICO";
		else
			$aRow[ $aColumns[7] ] = "OPERADOR";
		$row = array();
                $row[] = $aRow[ $aColumns[0] ];
                $row[] = $aRow[ $aColumns[7] ];
                $row[] = $aRow[ $aColumns[1] ];
