...
class vozcliente {
    ...
    public function vozcliente($id_vozcliente, $detalle,$contacto, $patente, $fecha) {
    ...
    public function listvozclientes($idvc = 0, $patente= "") {
        $data = array();
        $sql = "SELECT * FROM vozcliente";
        if($idvc != 0)
        { 
        	$sql = $sql . " WHERE id_vozcliente = " . $idvc;
		}
		else
		{
			$sql = $sql . " WHERE usada = 0";
		}
