...
//{
	...
	*/
	...
	$pdf->SetFont('Times','B',13);
	$pdf->setXY(5,150);
	$pdf->Cell(120,5,'Facturas de proveedor',1,0,'C');
	$pdf->Ln();
	$pdf->SetX(5);
	...
	foreach($fact_prov as $fact_p)
	{
		...
		$arr = explode('|', $fact_p);
		$pdf->SetX(5);
		$pdf->Cell(40,5,$arr[1],1,0,'C');
		$pdf->Cell(40,5,$arr[2],1,0,'C');
		$pdf->Cell(40,5,"$".$arr[3],1,0,'C');
		$total_fact_prov = $total_fact_prov + $arr[3];
	}
	for($i = 1 ; $i <= 5 - count($fact_prov) ; $i++)
	{
		$pdf->Ln();
		$pdf->SetX(5);
		$pdf->Cell(40,5,"",1,0,'C');
	...
	$pdf->SetFont('Times','B',13);
	$pdf->Cell(90,5,"TOTAL",1,0,'R');
	$pdf->Cell(30,5, "$" . $total_fact_prov, 1,0,'C');
	...
	for($i = 1 ; $i <= 10 - count($retenciones) ; $i++)
	{
		$pdf->Ln();
		$pdf->SetX(130);
		$pdf->Cell(25,5,"",1,0,'C');
	...
	$pdf->Ln();
	$pdf->SetXY(5, 265);
	$pdf->MultiCell(150,6,"Observaciones: " . $obs, $bd);
	...
	$pdf->SetX(160);
	$pdf->SetFont('Times','',10);
	$pdf->Cell(40,3,"p/Frenos Oeste S.A.",$bd,0, 'C');
...
$pdf->Output("impresiones/recibos/".$numero.".pdf","F");
$pdf->Output();
